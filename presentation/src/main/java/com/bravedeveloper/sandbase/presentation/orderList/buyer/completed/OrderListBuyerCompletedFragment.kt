...
@AndroidEntryPoint
class OrderListBuyerCompletedFragment
    ...
    override val adapter =
        CompositeDelegateAdapter(OrderListBuyerCompletedDelegateAdapter({
            it.number?.let { number -> goToOrder(number) }
        }, {}, context))

    override fun addOrdersToAdapter(orders: List<Order>) {
        val newOrders = mutableListOf<OrderItemBuyerCompleted>()
        orders.map { order ->
            newOrders.add(
                OrderItemBuyerCompleted(
